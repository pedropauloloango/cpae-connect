...
BEGIN
  ...
  IF v_inscrito_id IS NULL THEN
    RETURN QUERY SELECT false, 'CPF não encontrado entre os inscritos deste ano do curso.'::TEXT, NULL::TEXT, false;
    RETURN;
  END IF;
  ...
  INSERT INTO public.saude_mental_presencas (encontro_id, inscrito_id, cpf_informado, origem, registrado_ip)
  VALUES (v_encontro_id, v_inscrito_id, v_cpf, 'qrcode', v_ip);
