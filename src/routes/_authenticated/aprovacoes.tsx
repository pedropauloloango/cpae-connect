...
import { meetingNumberLabels, reportStatusLabels } from "@/lib/labels";
  ...
  const { data: pending = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("meetings")
        .select("id, numero, status, data_atendimento, relato_texto, submitted_at, request:requests!inner(id, numero, aluno_nome, school_nome_snapshot), professional:professionals(nome)")
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {(pending as unknown as Array<{ id: string; numero: string; data_atendimento: string; relato_texto: string | null; submitted_at: string; status: string; request: { id: string; numero: string; aluno_nome: string; school_nome_snapshot: string }; professional: { nome: string } | null }>).map((m) => (
          <Card key={m.id}>
            <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
              ...
              <Badge variant="outline">{reportStatusLabels[m.status]}</Badge>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="text-xs text-muted-foreground">Atendimento: {new Date(m.data_atendimento).toLocaleString("pt-BR")}</div>
              <div className="whitespace-pre-wrap rounded-md bg-muted/40 p-3 text-sm">{m.relato_texto}</div>
              <div className="flex flex-wrap gap-2">
                ...
                <Button size="sm" variant="ghost" className="text-destructive hover:bg-destructive/10" onClick={() => {
                  ...
                  if (c) decide.mutate({ meetingId: m.id, requestId: m.request.id, decision: "rejeitado", comentario: c });
                }} disabled={decide.isPending}>
