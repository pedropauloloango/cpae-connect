...
COMMENT ON COLUMN public.saude_mental_encontros.qr_ativo IS
  'Quando true, a URL pública aceita confirmação de presença.';
...
-- Public lookup (active QR only): returns minimal encontro info
...
-- Confirm presence by CPF via QR
...
BEGIN
  ...
  IF v_encontro_id IS NULL THEN
    RETURN QUERY SELECT false, 'QR Code inválido ou desativado para este encontro.'::TEXT, NULL::TEXT, false;
    RETURN;
  END IF;
